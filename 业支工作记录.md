...
inner join zt_user b on a.user_id=b.user_id where cert_type in(22,19,24,50,57) and a.region_id=792 and to_char(b.create_date,'yyyymm') = '202106';
...
```sql
SQL> view DNP_HJJ_RB1
```

```sql

drop table crm3_hzh_fee1;
create table crm3_hzh_fee1 as
select distinct a.phone_id,z.fee 一月消费,x.fee 二月消费,c.fee 三月消费,b.fee_1 六月消费,b.fee_2 五月消费,b.fee_3 四月消费
from 
(select distinct TU1627629900586_KEY phone_id from datamart.U1627629900586) a left join 
crm3_zt_sa b on a.phone_id = b.phone_id
left join crm3_dnp_bill_202101 z on a.phone_id = z.bill_id
left join crm3_dnp_bill_202102 x on a.phone_id = x.bill_id
left join crm3_dnp_bill_202103 c on a.phone_id = c.bill_id


select distinct a.*,q.rating_res 一月流量,w.rating_res 二月流量,e.rating_res 三月流量,r.rating_res 四月流量,t.rating_res 五月流量,y.rating_res 六月流量 
from 
crm3_hzh_fee1 a 
left join  crm3_zt_sa b on a.phone_id = b.phone_id
left join  zt_gprs_202101 q on a.phone_id = q.user_number
left join  zt_gprs_202102 w on a.phone_id = w.user_number
left join  zt_gprs_202103 e on a.phone_id = e.user_number
left join  zt_gprs_202104 r on a.phone_id = r.user_number
left join  zt_gprs_202105 t on a.phone_id = t.user_number
left join  zt_gprs_202106 y on a.phone_id = y.user_number
```

### 八月三号

```sql
江西省电力有限公司瑞昌市供电分公司物联卡号码 IP
select distinct TU1627885877449_KEY phone_id from datamart.U1627885877449

select b.phone_id,a.apn_ip from SF.INSX_APN_IP@FROM_CRMDBA1_CLONE a right join 
(select distinct TU1627885877449_KEY phone_id from datamart.U1627885877449) b on a.bill_id = b.phone_id
```

```sql
关于2021年7月集客量酬核算基础数据提取
drop table hzh_tmp10;
create table hzh_tmp10 as
select distinct a.county_code,a.user_id,a.phone_id,b.group_id,b.party_name group_name,b.group_level,b.manager_id,b.bill_id,b.staff_name
from ylp_zsy_2021paizhao_tmp2 a 
left join zt_jk_user_202107 b on a.user_id=b.user_id;
...
select distinct a.*,b.arpu_x arpu_dy,c.arpu_x arpu_sy,d.arpu_x arpu_s4
from hzh_tmp12 a
left join datamart.mart_group_fee_202107 b on a.user_id=b.user_id
left join datamart.mart_group_fee_202106 c on a.user_id=c.user_id
left join (select user_id,round(avg(arpu_x),2) arpu_x from (
select * from hjj_bill_202012_tcxtcz union all
select * from hjj_bill_202011_tcxtcz union all
select * from hjj_bill_202010_tcxtcz) group by user_id) d on a.user_id=d.user_id;

select * from datamart.mart_group_fee_202107
...
case when d.serv_id is not null then 1 else 0 end flag_kd,e.if_jtsx
from hzh_tmp13 a
 left join (select serv_id from xwp_wc_zy_202107 a inner join datamart.mart_int_91005_ms b on substr(a.imei,1,8)=b.tac
 where b.is_5g=1) b on a.user_id=b.serv_id
 left join crm3_dnp_user_yjt_202107 c on a.user_id=c.user_id
 left join zt_user_broadband_202107 d on a.user_id=d.user_id
 left join mart_qqt_jsk_detail_dm e on a.phone_id=e.phone_no;
...
i Number;
begin
  Execute Immediate 'Truncate Table crm3_hzh_familyPlanning';
b_date:='20210201';
e_date:='20210531';
m:=to_date(e_date,'yyyymmdd')-to_date(b_date,'yyyymmdd');
Loop
p_bill_date  := to_char(to_date(e_date,'yyyymmdd')-m,'yyyymmdd');
 execute immediate 'insert into crm3_hzh_familyPlanning 
 select start_time,opp_number,opp_hplmn2
from jf.dr_ismg_l_'||p_bill_date||'@jfxdcx  where user_number=''99273069008''';
 commit;
 Exit When m=0;
  m:=m-1;
  End Loop;
...
select * from jf.dr_ismg_l_20210212@jfxdcx a where user_number = '99273069008'
--归属地查询
select * from xg.sys_city@from_ggkdb1_clone

drop table crm3_hzh_familyPlanning;
create table crm3_hzh_familyPlanning(start_time varchar(60),opp_number varchar(60),opp_hplmn2 varchar(60));

select a.start_time,a.opp_number,b.city_name from crm3_hzh_familyPlanning a left join xg.sys_city@from_ggkdb1_clone b on a.opp_hplmn2 = b.city_code
```

```
物联网智能产品电摩卫士押金互斥明细
select q.* from 
(select distinct a.user_id from so1.ins_prod_792@from_crmdba1_clone a 
right join (select * from product.up_item_kind_rel@from_ggkdb1_clone where kind_id not in ('195110000292')) b
on a.prod_id = b.product_item_id) p 
inner join (select user_id,county_code,phone_id,state_name,order_name,create_date,organize_name,org_name_ck,
fee_1 七月消费, fee_2 六月消费, fee_3 五月消费, org_id,kg_name,kg_phone
from crm3_zt_sa where weigth > 31) q
on p.user_id = q.user_id
```
...
```sql
柘林水库
select TU16281473347060 phone_id,TU16281473347061 用户编号, TU1628147334706_KEY 账户编号 from datamart.U1628147334706
...
select * from zt_unpay_fee1 where  bill_id = '07927130105'

drop table crm3_hzh_test0;
create table crm3_hzh_test0 as
select distinct  a.phone_id,c.billing_cycle_id,sum(c.unpay_fee) unpay_fee from 
(select TU16281473347060 phone_id,TU16281473347061 用户编号, TU1628147334706_KEY 账户编号 from datamart.U1628147334706) a left join 
zt_unpay_fee1 c on a.用户编号 = c.user_id group by a.phone_id,c.billing_cycle_id 

select * from zt_unpay_fee1
select * from crm3_hzh_test

select phone_id,case when unpay_fee = min(billing_cycle_id) then billing_cycle_id else null end from 
crm3_hzh_test group by phone_id

drop table crm3_hzh_test01;
create table crm3_hzh_test01 as
select a.phone_id,
max(case billing_cycle_id when 202105 then unpay_fee/100 else 0 end) as 五月欠费,
max(case billing_cycle_id when 202106 then unpay_fee/100 else 0 end) as 六月欠费,
max(case billing_cycle_id when 202107 then unpay_fee/100 else 0 end) as 七月欠费 from crm3_hzh_test0 a  group by a.phone_id

select * from crm3_hzh_test01

drop table crm3_hzh_test04;
create table crm3_hzh_test04 as
select phone_id, 五月欠费,六月欠费,七月欠费,
case when 五月欠费 = greatest(五月欠费,六月欠费,七月欠费) then '五月' 
  when 六月欠费 = greatest(五月欠费,六月欠费,七月欠费) then '六月' 
  when 七月欠费 = greatest(五月欠费,六月欠费,七月欠费) then '七月' else '0' end 最大欠费月份,   
case when 五月欠费 = least(五月欠费,六月欠费,七月欠费) then '五月' 
  when 六月欠费 = least(五月欠费,六月欠费,七月欠费) then '六月' 
  when 七月欠费 = least(五月欠费,六月欠费,七月欠费) then '七月' else '0' end 最小欠费月份 from crm3_hzh_test01


select * from crm3_hzh_test04

select m.*,n.*,b.实时余额 from (select TU16281473347060 phone_id,TU16281473347061 用户编号, TU1628147334706_KEY 账户编号 from datamart.U1628147334706) m
left join crm3_hzh_test04 n on m.phone_id = n.phone_id
left join crm3_zt_bill_fee_now b on m.phone_id = b.phone_id
...
```sql
武宁县文旅新局过境短信
drop table crm3_hzh_bureauOfCulture1;
create table crm3_hzh_bureauOfCulture1(start_time varchar(60),opp_number varchar(60),opp_hplmn2 varchar(60));
...
i Number;
begin
  Execute Immediate 'Truncate Table crm3_hzh_bureauOfCulture';
b_date:='20210401';
e_date:='20210630';
m:=to_date(e_date,'yyyymmdd')-to_date(b_date,'yyyymmdd');
Loop
p_bill_date  := to_char(to_date(e_date,'yyyymmdd')-m,'yyyymmdd');
 execute immediate 'insert into crm3_hzh_bureauOfCulture
 select start_time,opp_number,opp_hplmn2
from jf.dr_ismg_l_'||p_bill_date||'@jfxdcx  where user_number=''10004038634''';
 commit;
 Exit When m=0;
  m:=m-1;
  End Loop;
...
i Number;
begin
  Execute Immediate 'Truncate Table crm3_hzh_bureauOfCulture1';
b_date:='20210601';
e_date:='20210630';
m:=to_date(e_date,'yyyymmdd')-to_date(b_date,'yyyymmdd');
Loop
p_bill_date  := to_char(to_date(e_date,'yyyymmdd')-m,'yyyymmdd');
 execute immediate 'insert into crm3_hzh_bureauOfCulture1 
 select start_time,opp_number,opp_hplmn2
from jf.dr_ismg_l_'||p_bill_date||'@to_xdcxk  where user_number=''10004038634''';
 commit;
 Exit When m=0;
  m:=m-1;
  End Loop;
 ...
 select a.start_time,a.opp_number,b.city_name from 
 (select * from crm3_hzh_bureauOfCulture union all  select * from crm3_hzh_bureauOfCulture1) a 
 left join xg.sys_city@from_ggkdb1_clone b on a.opp_hplmn2 = b.city_code
 
 
insert into crm3_hzh_pengzeCulture
select start_time,opp_number,opp_hplmn2 from jf.dr_ismg_l_20210625@to_xdcxk  where user_number='10004038634'
...
select a.start_time,a.opp_number,b.city_name from crm3_hzh_bureauOfCulture a left join xg.sys_city@from_ggkdb1_clone b on a.opp_hplmn2 = b.city_code
```

### 瑞昌市卫生健康委员会办理了过境短信业务，因疫情防控需求

```sql
select a.start_time,a.opp_number,b.city_name from 
 (select start_time,opp_number,opp_hplmn2 from jf.dr_ismg_l_20210806@to_xdcxk  where user_number='99272848929') a 
 left join xg.sys_city@from_ggkdb1_clone b on a.opp_hplmn2 = b.city_code
```

```sql
提取彭泽县供电公司2186张抄表卡2021年6月和7月份月的月出账金额
select a.*,b.fee_1 七月出账,b.fee_2 六月出账 from
(select TU16282345784180 company ,TU1628234578418_KEY phone_id from datamart.U1628234578418) a
left join crm3_zt_sa b on a.phone_id = b.phone_id
```

```sql
2021年7月集团单位证件开户数据
drop table hzh_temp0;
create table hzh_temp0 as
select cert_type,cert_code,a.user_id,b.phone_id,b.county_code,b.op_id,b.org_id,b.create_date,b.offer_id plan_id,b.order_name plan_name,b.user_type
from sf.insx_cert_user_rel_OTH@from_crmdba1_clone a 
inner join zt_user b on a.user_id=b.user_id where cert_type in(22,19,24,50,57) and a.region_id=792 and to_char(b.create_date,'yyyymm') = '202107';


drop table hzh_temp01;
create table hzh_temp01 as
select a.*,c.so_man 经办人,c.reg_nbr 经办人号,b.use_name 使用人,b.use_reg_nbr 使用号
from hzh_temp0 a
left join party.CM_CUSTOMER_INFO_EXT@from_ggkdb1_clone b on a.user_id=b.user_id
left join so1.insx_sogroup_operator_792@from_crmdba1_clone c on a.user_id=c.user_id;
select * from hzh_temp01;
```
...
```sql
中移铁通公司七月分税率
select * from crm3_acc_bill_dtl_0792202107 where acct_id = '400138459956'
```

```sql
国网江西省电力有限公司瑞昌市供电公司统付4月-7月分税率
drop table crm3_hzh_07;
create table crm3_hzh_07 as
select b.* from 
(select distinct TU1629081112616_KEY phone_id from datamart.U1629081112616) a
left join crm3_acc_bill_dtl_0792202107 b on a.phone_id = b.phone_id

select phone_id,tax_rate,sum(fee) from crm3_hzh_07 group by phone_id,tax_rate
```

```sql
提取集团单位成员手机号码消费、套餐等情况
select distinct TU1629334702181_KEY phone_id from datamart.U1629334702181

select * from crm3_zt_sa where order_name like '%5G%'
select a.phone_id,case when b.order_name like '%5G%' then 1 else 0 end 是否5G套餐,b.order_name,b.is_zkyj 是否办理折扣活动,b.fee_3 五月消费,b.fee_2 六月消费,
b.fee_1 七月消费,f.flow 五月流量,g.flow 六月流量,h.flow 七月流量 from (select distinct TU1629334702181_KEY phone_id from datamart.U1629334702181) a
left join crm3_zt_sa b on a.phone_id = b.phone_id
left join  zt_user_fee_202105 f on a.phone_id = f.phone_id
left join  zt_user_fee_202106 g on a.phone_id = g.phone_id
left join  zt_user_fee_202107 h on a.phone_id = h.phone_id



select * from zt_jk_user_202105 where group_id = '87927927526'
```

```sql
集客集团证件相关数据提取
select a.start_time,a.opp_number,b.city_name from 
 (select start_time,opp_number,opp_hplmn2 from jf.dr_ismg_l_20210824@to_xdcxk  where user_number='99272848929') a 
 left join xg.sys_city@from_ggkdb1_clone b on a.opp_hplmn2 = b.city_code
 
 
 select * from zt_jk_user_202108 order by bill_month
 
 select a.county_code,a.group_seq,a.group_name,b.fee from zt_jk_user_202108 a
 left join yrx_xxh_income_op_2017 b on a.op_id = b.op_id
 
 
 --有效集团梳理
 select group_city 地市代码,city_name 地市名称,region_type 区县代码,county_name 区县名称,group_id 集团ID,group_seq 实体编号,
 group_name 集团名称,group_level 集团等级 from jtzj_qs where sf_yx is null;

---inner join 
select c.*,d.op_id from 
(select group_city 地市代码,city_name 地市名称,region_type 区县代码,county_name 区县名称,group_id 集团ID,group_seq 实体编号,
 group_name 集团名称,group_level 集团等级 from jtzj_qs where sf_yx is null) c
 inner join zt_jk_user_202108 d on c.集团id = d.group_id
 
select f.*,b.fee from  
( select c.*,d.op_id from 
(select group_city 地市代码,city_name 地市名称,region_type 区县代码,county_name 区县名称,group_id 集团ID,group_seq 实体编号,
 group_name 集团名称,group_level 集团等级 from jtzj_qs where sf_yx is null) c
 inner join zt_jk_user_202108 d on c.集团id = d.group_id) f 
 left join yrx_xxh_income_op_2017 b on f.op_id = b.op_id
 
 b.plan_type,b.bill_month,b.fee
 
 and bill_month like '%2021%'
```

### 记录一些表

```sql
查询流量：zt_gprs_202107、zt_user_fee_202107
查询余额结余：crm3_zt_bill_fee_now

jf.dr_ismg_l_'||p_bill_date||'@jfxdcx
jf.dr_ismg_l_20210625@to_xdcxk   六月以后找不到可以使用这个
...
注意：转十六进制：=DEC2HEX(G4)     转十进制： =DEC2HEX(H4,2)    注意转成十六进制要两位
 ...
 select * from zt_zk_dtl  折扣
 
 就是那个集团网对应的product_item_id，在哪个表里面能找到对应的字段匹配
 在so1.ins_prod_792@from_crmdba1_clone  这个表里面的prod_id能匹配
 crm3_acc_bill_dtl_0792202107  通过电话号码查税率
...
```sql
广州中数科技发展有限公司物联网卡号码数据（只有2021年流量数据）
drop table crm3_hzh_12m;
create table crm3_hzh_12m as
select a.*,e.rating_res 一月,f.rating_res 二月,g.rating_res 三月,h.rating_res 四月,i.rating_res 五月,j.rating_res 六月,k.rating_res 七月 from 
(select distinct TU1629687333379_KEY phone_id from datamart.U1629687333379) a
left join  zt_gprs_202101 e on a.phone_id = e.user_number
left join  zt_gprs_202102 f on a.phone_id = f.user_number
left join  zt_gprs_202103 g on a.phone_id = g.user_number
left join  zt_gprs_202104 h on a.phone_id = h.user_number
left join  zt_gprs_202105 i on a.phone_id = i.user_number
left join  zt_gprs_202106 j on a.phone_id = j.user_number
left join  zt_gprs_202107 k on a.phone_id = k.user_number

select * from crm3_hzh_12m

select a.*,b.state_name,c.其中上月普通结余,c.实时余额,c.其中上月专项结余 from 
crm3_hzh_12m a
left join zt_user b on a.phone_id = b.phone_id
left join crm3_zt_bill_fee_now c on a.phone_id = c.phone_id

select * from crm3_zt_bill_fee_now where phone_id = '1440346189759'
...
drop table crm3_hzh_test00;
create table crm3_hzh_test00 as
select distinct  a.phone_id,c.billing_cycle_id,sum(c.unpay_fee) unpay_fee from 
(select TU16261475370880 phone_id,TU16261475370881 用户编号, TU1626147537088_key 账户编号 from datamart.U1626147537088) a left join 
zt_unpay_fee1 c on a.用户编号 = c.user_id group by a.phone_id,c.billing_cycle_id 

select * from crm3_hzh_test00
...
max(case billing_cycle_id when 202105 then unpay_fee/100 else 0 end) as 五月欠费,
max(case billing_cycle_id when 202106 then unpay_fee/100 else 0 end) as 六月欠费,
max(case billing_cycle_id when 202107 then unpay_fee/100 else 0 end) as 七月欠费 from crm3_hzh_test00 a  group by a.phone_id

select * from crm3_hzh_test01

drop table crm3_hzh_test04;
create table crm3_hzh_test04 as
select phone_id, 四月欠费,五月欠费,六月欠费,七月欠费,
case when 四月欠费 = greatest(四月欠费,五月欠费,六月欠费,七月欠费) then '四月' 
     when 五月欠费 = greatest(四月欠费,五月欠费,六月欠费,七月欠费) then '五月' 
     when 六月欠费 = greatest(四月欠费,五月欠费,六月欠费,七月欠费) then '六月'
     when 七月欠费 = greatest(四月欠费,五月欠费,六月欠费,七月欠费) then '七月' else '0' end 最大欠费月份,  
  
case when 四月欠费 = least(四月欠费,五月欠费,六月欠费,七月欠费) then '四月' 
     when 五月欠费 = least(四月欠费,五月欠费,六月欠费,七月欠费) then '五月' 
     when 六月欠费 = least(四月欠费,五月欠费,六月欠费,七月欠费) then '六月'
     when 七月欠费 = least(四月欠费,五月欠费,六月欠费,七月欠费) then '七月' else '0' end 最小欠费月份 from crm3_hzh_test01
...
```sql
异常升档稽核数据
drop table crm3_hzh_Upshift;
create table crm3_hzh_Upshift as
select a.*,b.phone_id from 
(select * from crm3_dnp_plan_next_202107 where region_id=792 and old_offer_value<30 and offer_value>=128 ) a,zt_user b 
where a.user_id=b.user_id(+);

select a.*,b.state_name,b.is_zkyj,b.折扣押金到期时间,b.is_zdyj,b.zdyj_name,b.八月消费,b.七月消费,b.六月消费 
from crm3_hzh_Upshift a
left join 
(select phone_id,state_name,is_zkyj,zkyj_expire 折扣押金到期时间,is_zdyj,zdyj_name,fee_1 八月消费,fee_2 七月消费,fee_3 六月消费 from crm3_zt_sa) b
on a.phone_id = b.phone_id



select * from crm3_hzh_Upshift 

--主要代码在下面

select distinct a.*,x.八月流量,x.七月月流量,x.六月流量,case when e.产品名称 is not null then 1 else 0 end 是否办理云家庭礼包宽带,
e.活动办理时间 云家庭礼包宽带办理时间,f.create_date 折扣押金办理时间  from 
(select a.*,b.state_name,b.is_zkyj,zkyj_name,b.is_zdyj,b.zdyj_name,b.八月消费,b.七月消费,b.六月消费 
from crm3_hzh_Upshift a
left join 
(select phone_id,state_name,is_zkyj,zkyj_name,zkyj_expire 折扣押金到期时间,is_zdyj,zdyj_name,fee_1 八月消费,fee_2 七月消费,fee_3 六月消费 from crm3_zt_sa) b
on a.phone_id = b.phone_id) a
left join (select * from zt_user_broadband where 产品名称 like '%云家庭%') e on a.phone_id = e.号码
left join zt_zk_dtl f on a.user_id = f.user_id
left join ( select a.phone_id,sum(b.rating_res) 八月流量,sum(c.rating_res) 七月月流量,sum(d.rating_res) 六月流量 from 
crm3_hzh_Upshift a
left join zt_gprs_202108 b on a.phone_id = b.user_number
left join zt_gprs_202107 c on a.phone_id = c.user_number
left join zt_gprs_202106 d on a.phone_id = d.user_number
 group by a.phone_id) x on a.phone_id = x.phone_id


 select * from zt_zk_dtl
 
 
 select a.phone_id,sum(b.rating_res) 八月流量,sum(c.rating_res) 七月月流量,sum(d.rating_res) 六月流量 from 
crm3_hzh_Upshift a
left join zt_gprs_202108 b on a.phone_id = b.user_number
left join zt_gprs_202107 c on a.phone_id = c.user_number
left join zt_gprs_202106 d on a.phone_id = d.user_number
...
```sql
8月集客量酬核算基础数据提取
drop table hzh_tmp20;
create table hzh_tmp20 as
select distinct a.county_code,a.user_id,a.phone_id,b.group_id,b.party_name group_name,b.group_level,b.manager_id,b.bill_id,b.staff_name
from ylx_jtpz_user_202106 a 
left join zt_jk_user_202108 b on a.user_id=b.user_id;

drop table hzh_tmp21;
create table hzh_tmp21 as
select a.*,b.offer_id plan_id,b.order_name plan_name,b.state_name
from hzh_tmp20 a 
left join zt_user b on a.user_id=b.user_id;

drop table hzh_tmp22;
create table hzh_tmp22 as
select distinct a.*,case when b.user_id is not null then 1 else 0 end flag_5g
from hzh_tmp21 a
left join (select to_number(user_id) user_id from hzh_tmp21 where plan_id in(select offer_id from crm3_dnp_lc_plan_1 where offer_type='5G套餐')
) b on a.user_id=b.user_id;


drop table hzh_tmp23;
create table hzh_tmp23 as
select distinct a.*,b.arpu_x arpu_dy,c.arpu_x arpu_sy,d.arpu_x arpu_s4
from hzh_tmp22 a
left join datamart.mart_group_fee_202108 b on a.user_id=b.user_id
left join datamart.mart_group_fee_202107 c on a.user_id=c.user_id
left join (select user_id,round(avg(arpu_x),2) arpu_x from (
select * from hjj_bill_202012_tcxtcz union all
select * from hjj_bill_202011_tcxtcz union all
select * from hjj_bill_202010_tcxtcz) group by user_id) d on a.user_id=d.user_id;

--select * from datamart.mart_group_fee_202108


drop table hzh_Tmp24;
create table hzh_tmp24 as
select distinct a.*,
case when b.serv_id is not null then 1 else 0 end flag_5gzd,
case when c.user_id is not null then 1 else 0 end flag_yjt,
case when d.serv_id is not null then 1 else 0 end flag_kd,e.if_jtsx
from hzh_tmp23 a
 left join (select serv_id from xwp_wc_zy_202108 a inner join datamart.mart_int_91005_ms b on substr(a.imei,1,8)=b.tac
 where b.is_5g=1) b on a.user_id=b.serv_id
 left join crm3_dnp_user_yjt_202108 c on a.user_id=c.user_id
 left join zt_user_broadband_202108 d on a.user_id=d.user_id
 left join mart_qqt_jsk_detail_dm e on a.phone_id=e.phone_no;

select * from hzh_tmp24;
```

### 九月三号

```
铁塔公司物联网卡8月统付数据   电话号码前面的0去掉
select distinct v.*,c.col16,n.rating_res 八月流量 from 
(select distinct b.phone_id,fee_1 八月出账金额,imei,state_name,offer_id,order_name,zkyj_name,zkyj_expire from 
(select * from so1.INS_ACCREL_792@from_crmdba1_clone where pay_type=2 and acct_id = '400128141327') a
left join crm3_zt_sa b  on a.user_id = b.user_id) v
left join qdres.rs_obj_inst_used_11_792@NEW_BBCXDB c on v.phone_id = c.col29
left join zt_gprs_202108 n on v.phone_id = n.user_number
```

```
龙腾生物高科技有限公司
drop table crm3_hzh_LongtengBiology;
create table crm3_hzh_LongtengBiology(user_number varchar(60),start_time varchar(60),opp_number varchar(60),duration varchar(60));
...
i Number;
begin
  Execute Immediate 'Truncate Table crm3_hzh_LongtengBiology';
b_date:='20210801';
e_date:='20210831';
m:=to_date(e_date,'yyyymmdd')-to_date(b_date,'yyyymmdd');
Loop
p_bill_date  := to_char(to_date(e_date,'yyyymmdd')-m,'yyyymmdd');
 execute immediate 'insert into crm3_hzh_LongtengBiology
 select user_number,start_time,opp_number,duration
from (select b.user_number,b.start_time,b.opp_number,b.duration from jf.dr_gsm_792_'||p_bill_date||'@to_xdcxk b where user_number in 
(select TRIM(leading ''0'' FROM phone_id) phone_id from (select distinct TU1630649956069_KEY phone_id from datamart.U1630649956069)))';
 commit;
 Exit When m=0;
  m:=m-1;
  End Loop;
...
select * from crm3_hzh_LongtengBiology;
```

```
select t408.ORG_ID A15420051685467, t408.PHONE_ID A15420051685948, t408.USER_ID A15420051686122 from MART792.CRM3_ZT_SA t408, () UP62 where to_char(t408.USER_ID) = UP62.TU1631237013378_KEY


select distinct TU16312370133780 group_id,TU16312370133781 group_name, TU1631237013378_KEY group_seq from datamart.U1631237013378


select * from datamart.mart_cm_meb_ds

select * from datamart.mart_cm_grp_mag_ds
select * from datamart.mart_cm_meb_ds
select * from datamart.mart_cm_meb_ds

select * from crm3_dnp_czz_zxy_05

select a.*,b.group_iden_type,b.op_id,b.org_id from 
(select distinct TU16312370133780 group_id,TU16312370133781 group_name, TU1631237013378_KEY group_seq from datamart.U1631237013378)  a
left join (select * from datamart.mart_cust_heshangwu_mx_ds where city_id = '792') b
on a.group_seq = b.group_id

select * from datamart.mart_cust_heshangwu_mx_ds where city_id = '792' and group_id = 'JX9202794772'
```
...
```
全省工单
select user_id,offer_id,order_name,create_date,state,create_op_id,create_org_id,county_code,region_id,expire_date from so1.ins_offer_790@from_crmdba1_clone where offer_id in
('310095400633','310095400608','310095400609')  and expire_date
between to_date('20210801','yyyymmdd') and to_date('20210910','yyyymmdd') union all
select user_id,offer_id,order_name,create_date,state,create_op_id,create_org_id,county_code,region_id,expire_date  from so1.ins_offer_791@from_crmdba1_clone where offer_id in
('310095400633','310095400608','310095400609') and expire_date
between to_date('20210801','yyyymmdd') and to_date('20210910','yyyymmdd') union all
select user_id,offer_id,order_name,create_date,state,create_op_id,create_org_id,county_code,region_id,expire_date  from so1.ins_offer_792@from_crmdba1_clone where offer_id in
('310095400633','310095400608','310095400609') and expire_date
between to_date('20210801','yyyymmdd') and to_date('20210910','yyyymmdd') union all
select user_id,offer_id,order_name,create_date,state,create_op_id,create_org_id,county_code,region_id,expire_date  from so1.ins_offer_793@from_crmdba1_clone where offer_id in
('310095400633','310095400608','310095400609') and expire_date
between to_date('20210801','yyyymmdd') and to_date('20210910','yyyymmdd') union all
select user_id,offer_id,order_name,create_date,state,create_op_id,create_org_id,county_code,region_id,expire_date  from so1.ins_offer_794@from_crmdba1_clone where offer_id in
('310095400633','310095400608','310095400609') and expire_date
between to_date('20210801','yyyymmdd') and to_date('20210910','yyyymmdd') union all
select user_id,offer_id,order_name,create_date,state,create_op_id,create_org_id,county_code,region_id,expire_date  from so2.ins_offer_795@from_crmdbb1_clone where offer_id in
('310095400633','310095400608','310095400609') and expire_date
between to_date('20210801','yyyymmdd') and to_date('20210910','yyyymmdd') union all
select user_id,offer_id,order_name,create_date,state,create_op_id,create_org_id,county_code,region_id,expire_date  from so2.ins_offer_796@from_crmdbb1_clone where offer_id in
('310095400633','310095400608','310095400609') and expire_date
between to_date('20210801','yyyymmdd') and to_date('20210910','yyyymmdd') union all
select user_id,offer_id,order_name,create_date,state,create_op_id,create_org_id,county_code,region_id,expire_date  from so2.ins_offer_797@from_crmdbb1_clone where offer_id in
('310095400633','310095400608','310095400609') and expire_date
between to_date('20210801','yyyymmdd') and to_date('20210910','yyyymmdd') union all
select user_id,offer_id,order_name,create_date,state,create_op_id,create_org_id,county_code,region_id,expire_date  from so2.ins_offer_798@from_crmdbb1_clone where offer_id in
('310095400633','310095400608','310095400609') and expire_date
between to_date('20210801','yyyymmdd') and to_date('20210910','yyyymmdd') union all
select user_id,offer_id,order_name,create_date,state,create_op_id,create_org_id,county_code,region_id,expire_date  from so2.ins_offer_799@from_crmdbb1_clone where offer_id in
('310095400633','310095400608','310095400609') and expire_date
between to_date('20210801','yyyymmdd') and to_date('20210910','yyyymmdd') union all
select user_id,offer_id,order_name,create_date,state,create_op_id,create_org_id,county_code,region_id,expire_date  from so2.ins_offer_701@from_crmdbb1_clone where offer_id in
('310095400633','310095400608','310095400609');
```

```
某公司一到三月税率

drop table crm3_hzh_onetotree;
create table crm3_hzhonetothree as
select * from crm3_acc_bill_dtl_0792202101 where acct_id = '400050015230'
union all select * from crm3_acc_bill_dtl_0792202102 where acct_id = '400050015230'
union all select * from crm3_acc_bill_dtl_0792202103 where acct_id = '400050015230'

select phone_id,tax_rate,sum(fee) from crm3_hzh_07 group by phone_id,tax_rate

select * from crm3_acc_bill_dtl_0792202107

select * from crm3_acc_bill_dtl_0792202101 where acct_id = '400050015230'
union all select * from crm3_acc_bill_dtl_0792202102 where acct_id = '400050015230'
union all select * from crm3_acc_bill_dtl_0792202103 where acct_id = '400050015230'

select phone_id,tax_rate,bill_cycle,sum(fee) from crm3_hzhonetothree group by phone_id,tax_rate,bill_cycle
```

```
电摩卫士，涉及到身份证号的提取
drop table crm3_hzh_iddata;
create table crm3_hzh_iddata as
select a.*,b.state_name,b.user_id from 
(select distinct TU1631780902785_KEY phone_id from datamart.U1631780902785) a
left join zt_user b on a.phone_id = b.phone_id

drop table crm3_hzh_iddata2;
create table crm3_hzh_iddata2 as
select * from (
select t.cert_code iden_nr,user_id,cust_id,create_date,region_id from  sf.insx_cert_user_rel_1@from_crmdba1_clone  t   union all 
select t.cert_code iden_nr,user_id,cust_id,create_date,region_id  from  sf.insx_cert_user_rel_0@from_crmdba1_clone  t   union all 
select t.cert_code iden_nr,user_id,cust_id,create_date,region_id  from  sf.insx_cert_user_rel_2@from_crmdba1_clone  t   union all 
select t.cert_code iden_nr,user_id,cust_id,create_date,region_id  from  sf.insx_cert_user_rel_3@from_crmdba1_clone  t   union all 
select t.cert_code iden_nr,user_id,cust_id,create_date,region_id  from  sf.insx_cert_user_rel_4@from_crmdba1_clone  t   union all 
select t.cert_code iden_nr,user_id,cust_id,create_date,region_id  from  sf.insx_cert_user_rel_5@from_crmdba1_clone  t   union all 
select t.cert_code iden_nr,user_id,cust_id,create_date,region_id  from  sf.insx_cert_user_rel_6@from_crmdba1_clone  t   union all 
select t.cert_code iden_nr,user_id,cust_id,create_date,region_id  from  sf.insx_cert_user_rel_7@from_crmdba1_clone  t   union all 
select t.cert_code iden_nr,user_id,cust_id,create_date,region_id  from  sf.insx_cert_user_rel_8@from_crmdba1_clone  t   union all 
select t.cert_code iden_nr,user_id,cust_id,create_date,region_id  from  sf.insx_cert_user_rel_9@from_crmdba1_clone  t   union all 
select t.cert_code iden_nr,user_id,cust_id,create_date,region_id  from  sf.insx_cert_user_rel_X@from_crmdba1_clone  t  union all
select t.cert_code iden_nr,user_id,cust_id,create_date,region_id  from  sf.insx_cert_user_rel_OTH@from_crmdba1_clone  t)
where user_id in(select user_id from crm3_hzh_iddata); 

select a.phone_id,a.state_name,b.iden_nr from crm3_hzh_iddata a
left join crm3_hzh_iddata2 b on a.user_id = b.user_id
...
select distinct TU16318635225310 group_id,TU16318635225311 group_name,TU1631863522531_KEY group_seq from datamart.U1631863522531
```

```sql
德安县数据需求提取申请
select distinct TU16318666753320 phone_id,TU1631866675332_KEY name_id from datamart.U1631866675332

select * from crm3_hzh_xunyang3

drop table crm3_hzh_tempdean;
create table crm3_hzh_tempdean as 
select distinct a.*,five_type,d.user_id from (select distinct TU1631953442952_KEY phone_id from datamart.U1631953442952) a
left join 
(select case when phone_no1 is null then phone_no2 else phone_no1 end phone_no,if_yh,'A' five_type,if_jtw,if_jtvw from mart_five_ywzk_dm where city_code=792
union all 
select phone_no,if_yh,'B' five_type,if_jtw,if_jtvw from mart_five_wkyh_dm where city_id=792
...
select product_no as phone_id,if_yh,'D' five_type,if_jtw,if_jtvw from mart_five_gjz_dm where city_id=792
union all 
select case when phone_no1 is null then phone_no2 else phone_no1 end phone_no,if_yh,'E' five_type,if_jtw,if_jtvw from mart_five_gjz_dkc_dm) b on a.phone_id = b.phone_no
left join crm3_zt_sa d on a.phone_id = d.phone_id

select * from crm3_zt_sa

select distinct TU1631953442952_KEY phone_id from datamart.U1631953442952
--流量包
select g.*,h.*,case when g.user_id = h.user_id then 1 else 0 end 是否办理流量包 from crm3_hzh_tempdean g
left join 
(select user_id,order_name,create_date,expire_date from so1.ins_offer_792@from_crmdba1_clone where expire_date>sysdate and state=1 and offer_id in
('212080720876',
'212080720877',
'212080720878',
'212080720879',
...
'212080720857',
'212080720858',
'212080720859')) h on g.user_id = h.user_id

```


```sql
九江市工商行政管理局（集团名称、集团编号、集团短号）
select distinct a.phone_id,c.group_id,c.group_name,b.short_num from  
(select distinct TU1635753332271_KEY phone_id from datamart.U1635753332271) a
left join crm3_dnp_vpmn_6 b on a.phone_id = b.bill_id
left join zt_jk_user_202110 c on a.phone_id = c.phone_id order by a.phone_id

```

```sql
铁塔公司物联卡十月统付
select user_id,acct_id from so1.INS_ACCREL_792@from_crmdba1_clone where expire_date >= sysdate and acct_id = '400128141327'


drop table hzh_temp4;
create table hzh_temp4 as
select distinct a.*,b.flow,c.phone_id,c.state_name,c.offer_id plan_id,c.order_name plan_name,d.phone_id bill_id,e.imei
from (select user_id,acct_id from so1.INS_ACCREL_792@from_crmdba1_clone where expire_date >= sysdate and acct_id = '400128141327') a
left join (select user_id,round(sum(rating_res)/1024,2) flow from zt_gprs_202110 group by user_id) b on a.user_id=b.user_id
left join zt_user c on a.user_id=c.user_id
left join zt_user d on a.acct_id=d.acct_id
left join xwp_wc_zy_imei_202110 e on a.user_id=e.serv_id;    --需要改两个


/*select * from hzh_temp4;
select * from crm3_dnp_bill_202102_1;*/

--select * from xwp_wc_zy_imei_202110

drop table hzh_temp5;
create table hzh_temp5 as
select a.*,b.offer_id,b.order_name,b.expire_date
from hzh_temp4 a
left join (select * from so1.ins_offer_792@from_crmdba1_clone where offer_type='OFFER_VAS_PREPAY' and expire_date>sysdate) b on a.user_id=b.user_id;

drop table hzh_temp6;
create table hzh_temp6 as
select a.*,b.col16 sim,b.col15 imsi
from hzh_Temp5 a
left join qdres.rs_obj_inst_used_14_792@new_bbcxdb b on a.phone_id=b.serial_no;

--最终
select distinct a.*,b.fee_1 from hzh_temp6 a
left join crm3_zt_sa b on a.phone_id = b.phone_id;
